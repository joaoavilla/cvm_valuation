{#
  Balanço Patrimonial Passivo unificado. Fotografia: não tem DT_INI_EXERC.
...
con as (
    ...
    from {{ source('cvm_dfp_raw', 'bpp_con') }}
),

ind as (
    select *, 'INDIVIDUAL' as tipo_df
    from {{ source('cvm_dfp_raw', 'bpp_ind') }}
),
...
transformado as (
    select
        ...
        {{ unidade_da_conta('BPP', 'CD_CONTA') }}                        as unidade,
        {{ valor_em_reais('BPP', 'CD_CONTA', 'ESCALA_MOEDA', 'VL_CONTA') }} as valor
    from unificado
),
