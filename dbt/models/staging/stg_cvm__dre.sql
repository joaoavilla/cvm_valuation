...
con as (
    select *, 'CONSOLIDADO' as tipo_df from {{ source('cvm_dfp_raw', 'dre_con') }}
),

ind as (
    select *, 'INDIVIDUAL' as tipo_df from {{ source('cvm_dfp_raw', 'dre_ind') }}
),
...
qualify row_number() over (
    partition by cd_cvm, tipo_df, dt_referencia, cd_conta, ordem_exercicio
    order by valor
) = 1
