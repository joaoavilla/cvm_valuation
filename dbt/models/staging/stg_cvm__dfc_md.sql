{#
  DFC pelo método direto, unificada.
...
con as (
    ...
    from {{ source('cvm_dfp_raw', 'dfc_md_con') }}
),

ind as (
    select *, 'INDIVIDUAL' as tipo_df
    from {{ source('cvm_dfp_raw', 'dfc_md_ind') }}
),
...
transformado as (
    select
        ...
        cast(DT_INI_EXERC as date)         as dt_inicio_exercicio,   -- período REAL do valor
        cast(DT_FIM_EXERC as date)         as dt_fim_exercicio,
        CD_CONTA                           as cd_conta,
        DS_CONTA                           as ds_conta,
        ST_CONTA_FIXA = 'S'                as conta_fixa,
        {{ unidade_da_conta('DFC_MD', 'CD_CONTA') }}                        as unidade,
        ESCALA_MOEDA                       as escala_moeda,
        {{ valor_em_reais('DFC_MD', 'CD_CONTA', 'ESCALA_MOEDA', 'VL_CONTA') }} as valor
    from unificado
),
...
resolvido as (
    ...
    group by
        cnpj, cd_cvm, razao_social, tipo_df, versao, dt_referencia, ordem_exercicio, dt_inicio_exercicio,
